"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
var chai_1 = require("chai");
var app_1 = __importDefault(require("../src/app"));
var supertest_1 = require("supertest");
var token, userId;
var imageIds = [];
it('should POST /user/register and register a new user', function () {
    return __awaiter(this, void 0, void 0, function () {
        var res;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, (0, supertest_1.agent)(app_1.default)
                        .post('/user/register').send({
                        first_name: 'Jason',
                        last_name: 'Liu',
                        email: 'imagestest@gmail.com',
                        password: 'password'
                    })];
                case 1:
                    res = _a.sent();
                    token = res.body.token;
                    userId = res.body.id;
                    (0, chai_1.expect)(res.status).to.equal(201);
                    (0, chai_1.expect)(res.body).to.be.an('object');
                    return [2 /*return*/];
            }
        });
    });
});
it('upload 1 image file', function () {
    return __awaiter(this, void 0, void 0, function () {
        var res;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, (0, supertest_1.agent)(app_1.default).post("/images/" + userId)
                        .set('x-access-token', token)
                        .attach('image', 'images/alex-seinet-8dmtbpJJJ7k-unsplash.jpg')];
                case 1:
                    res = _a.sent();
                    (0, chai_1.expect)(res.status).to.equal(200);
                    return [2 /*return*/];
            }
        });
    });
});
it('upload 3 image files', function () {
    return __awaiter(this, void 0, void 0, function () {
        var res, _i, _a, image;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0: return [4 /*yield*/, (0, supertest_1.agent)(app_1.default).post("/images/" + userId)
                        .set('x-access-token', token)
                        .attach('image', 'images/alex-seinet-8dmtbpJJJ7k-unsplash.jpg')
                        .attach('image', 'images/chad-madden-22u9dunCITQ-unsplash.jpg')
                        .attach('image', 'images/christian-bowen-3JqVQsShG28-unsplash.jpg')];
                case 1:
                    res = _b.sent();
                    for (_i = 0, _a = res.body.images; _i < _a.length; _i++) {
                        image = _a[_i];
                        imageIds.push(image.public_id);
                    }
                    (0, chai_1.expect)(res.status).to.equal(200);
                    return [2 /*return*/];
            }
        });
    });
});
it('upload 10 image files', function () {
    return __awaiter(this, void 0, void 0, function () {
        var res;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, (0, supertest_1.agent)(app_1.default).post("/images/" + userId)
                        .set('x-access-token', token)
                        .attach('image', 'images/alex-seinet-8dmtbpJJJ7k-unsplash.jpg')
                        .attach('image', 'images/chad-madden-22u9dunCITQ-unsplash.jpg')
                        .attach('image', 'images/christian-bowen-3JqVQsShG28-unsplash.jpg')
                        .attach('image', 'images/clarisse-meyer-d6pLNFVZt_4-unsplash.jpg')
                        .attach('image', 'images/katarzyna-grabowska-sRAWQyoUiVQ-unsplash.jpg')
                        .attach('image', 'images/lucas-cleutjens-wRHbuIa9Pd0-unsplash.jpg')
                        .attach('image', 'images/marek-mucha-j14F47I7NO0-unsplash.jpg')
                        .attach('image', 'images/nathalia-segato-QIAtKF3pWqM-unsplash.jpg')
                        .attach('image', 'images/nathan-dumlao-EWD9bCDW0EE-unsplash.jpg')
                        .attach('image', 'images/nathan-riley-_ir1D49PRqM-unsplash.jpg')];
                case 1:
                    res = _a.sent();
                    (0, chai_1.expect)(res.status).to.equal(200);
                    return [2 /*return*/];
            }
        });
    });
});
it('get 1 uploaded photo of logged in user', function () {
    return __awaiter(this, void 0, void 0, function () {
        var res, images;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, (0, supertest_1.agent)(app_1.default).get("/images/" + userId)
                        .set('x-access-token', token)
                        .send({ imageIds: [imageIds[0]] })];
                case 1:
                    res = _a.sent();
                    images = res.body.resources;
                    (0, chai_1.expect)(images.length).to.be.equal(1);
                    (0, chai_1.expect)(res.status).to.equal(200);
                    (0, chai_1.expect)(res.body).to.be.an('object');
                    return [2 /*return*/];
            }
        });
    });
});
it('get 3 uploaded photos of logged in user', function () {
    return __awaiter(this, void 0, void 0, function () {
        var res, images;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, (0, supertest_1.agent)(app_1.default).get("/images/" + userId)
                        .set('x-access-token', token)
                        .send({ imageIds: imageIds })];
                case 1:
                    res = _a.sent();
                    images = res.body.resources;
                    (0, chai_1.expect)(images.length).to.be.equal(3);
                    (0, chai_1.expect)(res.status).to.equal(200);
                    (0, chai_1.expect)(res.body).to.be.an('object');
                    return [2 /*return*/];
            }
        });
    });
});
it('get all uploaded photos of logged in user', function () {
    return __awaiter(this, void 0, void 0, function () {
        var res, images;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, (0, supertest_1.agent)(app_1.default).get("/images/" + userId)
                        .set('x-access-token', token)];
                case 1:
                    res = _a.sent();
                    images = res.body.resources;
                    (0, chai_1.expect)(images.length).to.be.equal(14);
                    (0, chai_1.expect)(res.status).to.equal(200);
                    (0, chai_1.expect)(res.body).to.be.an('object');
                    return [2 /*return*/];
            }
        });
    });
});
it('delete 3 images with ids', function () {
    return __awaiter(this, void 0, void 0, function () {
        var res;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, (0, supertest_1.agent)(app_1.default)
                        .delete("/images/delete/" + userId)
                        .set('x-access-token', token)
                        .send({
                        imageIds: imageIds
                    })];
                case 1:
                    res = _a.sent();
                    (0, chai_1.expect)(res.status).to.equal(200);
                    (0, chai_1.expect)(res.body).to.be.an('object');
                    return [2 /*return*/];
            }
        });
    });
});
it('get all uploaded photos of logged in user after deletion', function () {
    return __awaiter(this, void 0, void 0, function () {
        var res, images;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, (0, supertest_1.agent)(app_1.default).get("/images/" + userId)
                        .set('x-access-token', token)];
                case 1:
                    res = _a.sent();
                    images = res.body.resources;
                    (0, chai_1.expect)(images.length).to.be.equal(11);
                    (0, chai_1.expect)(res.status).to.equal(200);
                    (0, chai_1.expect)(res.body).to.be.an('object');
                    return [2 /*return*/];
            }
        });
    });
});
it('should delete registered user', function () {
    return __awaiter(this, void 0, void 0, function () {
        var res;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, (0, supertest_1.agent)(app_1.default)
                        .delete('/user/delete')
                        .set('x-access-token', token).send({
                        email: 'imagestest@gmail.com',
                        password: 'password'
                    })];
                case 1:
                    res = _a.sent();
                    (0, chai_1.expect)(res.status).to.equal(200);
                    (0, chai_1.expect)(res.body).to.be.an('object');
                    return [2 /*return*/];
            }
        });
    });
});
//# sourceMappingURL=ImagesController.test.js.map